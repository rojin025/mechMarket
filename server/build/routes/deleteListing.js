...
export const deleteListingRoute = {
  ...
  handler: async (req, h) => {
    try {
      ...
      if (results.length === 0) {
        return Boom.forbidden("You do not have permission to delete this listing.");
      }
      const deleteResult = await db.query(`DELETE FROM listings WHERE id=? AND user_id=?`, [id, userId]);
      ...
      return h.response({
        message: "listing deleted sunccessfully"
      }).code(200);
    } catch (error) {
